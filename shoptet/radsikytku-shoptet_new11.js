(() => {
  ...
  const deliveryBox = () => {
    ...
    const validate = () => {
      ...
      // ✅ osobní odběr – časy podle dne:
      // Po–Pá: 10–12,12–14,14–16,16–18
      // So: 10–12,12–14
      // Ne: nelze
      if (isPickup()) {
        const wd = date ? new Date(date + "T00:00:00").getDay() : -1; // 1=Po ... 6=So,0=Ne

        // ❌ Neděle zavřeno
        if (wd === 0) {
          warn.style.display = "block";
          warn.textContent = "Osobní odběr v neděli není možný (zavřeno).";
          lock.setDelivery(false);
          return;
        }
        ...
        // ✅ Po–Pá
        if (wd >= 1 && wd <= 5) {
          help.textContent = "Osobní odběr: Po–Pá 10:00–18:00.";
        ...
        // ✅ Sobota: jen do 14:00
        if (wd === 6) {
          help.textContent = "Osobní odběr: sobota 10:00–14:00.";
          setTimeOptions(isToday);

          // povolit jen 10–12 a 12–14
          qa("option", t).forEach((o) => {
            if (!o.value) return;
            if (o.value.indexOf("14:00 – 16:00") >= 0 || o.value.indexOf("16:00 – 18:00") >= 0)
              o.disabled = true;
          });

          // když user měl uložený neplatný čas, vynuluj
          if (t.value && (t.value.indexOf("14:00 – 16:00") >= 0 || t.value.indexOf("16:00 – 18:00") >= 0)) {
            t.value = "";
            sessionStorage.setItem(RK.DT, "");
          }
  ...
  // ============================
  // ✅ Propsání do poznámky objednávky (+ aktivace checkboxu)
  // ============================
  const orderNote = () => {
    ...
    document.addEventListener("shoptet.content.updated", apply);

    const form = q("form#order-form") || q("form");
    if (form && !form.dataset.rkNoteSubmit) {
      form.dataset.rkNoteSubmit = "1";
