(() => {
  ...
  const page = {
    // ✅ FIX: Shoptet detail produktu má "type-detail", ale často nemá "in-detail"
    // nejspolehlivější je detekce přes existenci formuláře
    prod: () => !!q("#product-detail-form"),
    step1: () =>
      q("body") &&
      (q("body").classList.contains("in-krok-1") || !!q(".ordering-process")),
    step2: () =>
  ...
  // - správný vzhled (date + select)
  // - validace: kurýr od zítřka a bez víkendů
  // - validace: osobní odběr dnes jen budoucí čas
  // ============================
  const deliveryBox = () => {
    ...
    on(document, "change", (e) => {
      if (e.target && e.target.name === "shippingId") shipUpdate();
    });
  ...
  // ============================
  // ✅ PSČ kontrola – jen když v rekapitulaci je "mykurýr"
  // ============================
  const zipCheck = () => {
    ...
    const needZip = () => {
      const recap = q("#shipping-billing-summary") || q("#summary-box");
